...
{else}
        <BizField Name="{$fld.name}" Column="{$fld.col}" Type="{$fld.type}" {if $fld.length }Length="{$fld.length}"{/if} {if $fld.name != "Id"} Required="{if $fld.nullable }N{else}Y{/if}"{/if} {if $fld.format }Format="{$fld.format}"{/if} />
{/if}
{/foreach}