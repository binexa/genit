...
{/literal}        
        ...
        $do = $this->getDataObj();
		$do->updateRecords("[is_default]=0","[{$link_do_parent_id}]='$parent_id'");
		$do->updateRecords("[is_default]=1","[{$link_do_parent_id}]='$parent_id' and [Id]='$rec_id'");
...
{literal}
    public function increaseSortOrder($rec_id=null, $fieldName='priority')
    {
...
{/literal}
        $do = $this->getDataObj();
        $do->updateRecords("[$fieldName]=[$fieldName]+1", "[Id]='$rec_id'");
		$this->UpdateForm();
{literal}
    }
{/literal}    
...
{literal}
    public function decreaseSortOrder($rec_id=null,$fieldName='priority')
    {
        $fieldName = trim($fieldName);
		if ($rec_id==null) {
			$rec_id = (int)BizSystem::clientProxy()->getFormInputs('_selectedId');
...
{/literal}
        $do = $this->getDataObj();
        $do->updateRecords("[$fieldName]=[$fieldName]-1","[Id]='$rec_id' and [$fieldName] > 1");
		$this->UpdateForm();
{literal}
    }
{/literal}
